...
namespace WatchersNET.DNN.Modules
{
    ...
    public partial class SiteMapSl : SkinObjectBase
    {
        ...
        private List<TabInfo> FillTabs()
        {
            ...
            // Add Portal Tabs
            foreach (TabInfo objPortalTab in TabController.GetTabsBySortOrder(this.PortalSettings.PortalId).Select(objTab => objTab.Clone()))
            {
                ...
                case "custom":
                    {
                        if (this.taxVocabularies != null)
                        {
                            ...
                            foreach (string sVocabularyId in this.taxVocabularies)
                            {
